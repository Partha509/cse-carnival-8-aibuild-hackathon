...
- **Implemented campus tools:** 6/9 — the read tools `get_schedule`, `get_next_class`, `get_assignments`, `get_announcements`, `get_events`, `check_room_availability` (Task 6). Each validates params with zod and calls the backend service layer (`src/services/*`) via `fromService()`; none query Supabase directly. Action tools (`book_room`, `register_for_event`, `cancel_registration`) pending Task 7. The utility `get_current_datetime` also exists.
- **Tested:** 27 vitest unit tests (agent loop, datetime, 6 read tools with mocked services) + live agent smoke: correct tool selection and parameter/date extraction; backend errors relayed without fabrication.
- **Known Issues:** Full live-data verification needs Supabase env vars configured + `npm run seed`; without them the read tools return a config error which the agent relays safely.
